...
export function useDraftMenu(options: UseDraftMenuOptions): UseDraftMenuResult {
  ...
  useEffect(() => {
    const off = onChanged((payload: { schemaKey: string; setupId: string }) => {
      console.debug('[menu] onChanged', payload, { for: { schemaKey, selectedId } });
